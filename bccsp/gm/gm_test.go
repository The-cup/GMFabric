...
func TestKeyGenInvalidInputs(t *testing.T) {
	...
	csp, err := NewWithParams(-1, "SM", &mocks.KeyStore{StoreKeyErr: errors.New("cannot store key")})
...
func TestKeyDerivInvalidInputs(t *testing.T) {
	...
	_, err = csp.KeyDeriv(nil, &bccsp.SM2ReRandKeyOpts{})
	assert.Error(t, err)
	...
	_, err = csp.KeyDeriv(&mocks.MockKey{}, &bccsp.SM2ReRandKeyOpts{})
	assert.Error(t, err)
...
func TestKeyImportInvalidInputs(t *testing.T) {
	csp, err := NewWithParams(-1, "SM", &mocks.KeyStore{})
	assert.NoError(t, err)
...
func TestGetKeyInvalidInputs(t *testing.T) {
	...
	csp, err := NewWithParams(-1, "SM", &mocks.KeyStore{GetKeyErr: errors.New("cannot get key")})
	assert.NoError(t, err)
	...
	k := &mocks.MockKey{}
	csp, err = NewWithParams(-1, "SM", &mocks.KeyStore{GetKeyValue: k})
	assert.NoError(t, err)
